...
$stmt_cat = $pdo->prepare("SELECT id, nombre FROM Categorias WHERE usuario_id = ? AND tipo = 'gasto'");
$stmt_cat->execute([$id_usuario]);
$categorias = $stmt_cat->fetchAll();

// --- LÓGICA DE PAGINACIÓN (SOLO PÁGINA 1) ---
...
// CORRECCIÓN: Apuntar a 'Gastos' y usar alias 'g'
...
$stmt_gastos->bindParam(':offset', $offset, PDO::PARAM_INT);
...
<head>
    ...
    <title>Panel de Finanzas - Gastos</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <link rel="stylesheet" href="style/estilo_gastos.css">
</head>
<body>
...
<div class="wrapper">
    <nav class="sidebar p-3">
        ...
        <ul class="nav nav-pills flex-column gap-2">
            ...
            <li class="nav-item"><a href="gastos.php" class="nav-link active"><i class="bi bi-arrow-up-circle-fill me-2"></i> Gastos</a></li>
    ...
    <main class="main-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-principal" data-bs-toggle="modal" data-bs-target="#modalGasto" data-accion="agregar">
                <i class="bi bi-plus-circle-fill me-2"></i>Agregar Gasto
            </button>
        </div>
        ...
        <div class="card">
            <table class="tabla-finanzas">
                ...
                <tbody id="tabla-gastos-body">
                    <?php if (empty($gastos)): ?>
                        <tr><td colspan="5" class="text-center">No hay gastos registrados.</td></tr>
                    <?php else: ?>
                        <?php foreach ($gastos as $gasto): ?>
                        <tr>
                            <td class="fw-bold text-danger">$<?= htmlspecialchars(number_format($gasto['monto'], 2)) ?></td>
                            <td><?= htmlspecialchars($gasto['categoria_nombre']) ?></td>
...
<div class="modal fade" id="modalGasto" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title" id="modalGastoLabel"></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <form action="api/crud_gastos.php" method="POST">
...
<div class="modal fade" id="modalConfirmarEliminar" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="api/crud_gastos.php" method="POST">
                    <input type="hidden" name="gastoId" id="eliminarGastoId">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(modalGasto) {
        modalGasto.addEventListener('show.bs.modal', function (event) {
            ...
            const modalTitle = modalGasto.querySelector('.modal-title');
            const form = modalGasto.querySelector('form');
            form.reset();

            form.querySelector('#accion').value = accion;

            if (accion === 'modificar') {
                modalTitle.textContent = 'Modificar Gasto';
                form.querySelector('#gastoId').value = button.getAttribute('data-id');
                ...
                modalTitle.textContent = 'Agregar Nuevo Gasto';
    ...
    if(modalConfirmarEliminar) {
        modalConfirmarEliminar.addEventListener('show.bs.modal', function (event) {
            ...
            const inputEliminarId = modalConfirmarEliminar.querySelector('#eliminarGastoId');
            inputEliminarId.value = gastoId;
        });
    }
});
</script>
<script src="js/paginador_gastos.js"></script>
</body>
</html>